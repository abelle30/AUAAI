...
<script src="https://www.google.com/recaptcha/api.js" async defer></script>
        ...
        <title>AUAAI-News & Announcements</title>
...
<div id="heading-breadcrumbs" style="background-color: rgb(200, 211, 230)">
  ...
  <!--START CONTENT-->
  <div class="container-fluid px-5" style="margin-top: 1%;">
...
<br><br> 
  <div class="row">
    ...
    <?php 
        ...
        if($_GET['typid']!=''){
$_SESSION['typid']=intval($_GET['typid']);
}
  ...
  $total_pages_sql = "SELECT COUNT(*) FROM newsandannouncements";
  $result = mysqli_query($conn,$total_pages_sql);
  $total_rows = mysqli_fetch_array($result)[0];
  $total_pages = ceil($total_rows / $no_of_records_per_page);


  $query = mysqli_query($conn, "SELECT newsandannouncements.id as pid,newsandannouncements.newstitle,newsandannouncements.author,newsandannouncements.date,newsandannouncements.image,newsandannouncements.time,newsandannouncements.newsdescription,newsfilter.FilterName, newsfilter.id as cid 
  FROM newsandannouncements 
  LEFT JOIN newsfilter ON newsfilter.id = newsandannouncements.typeid 
  WHERE newsandannouncements.typeid = '" . $_SESSION['typid'] . "' 
  ORDER BY newsandannouncements.id DESC LIMIT $offset, $no_of_records_per_page");
...
?>
      <div class="card mb-9" style="max-width: 700%;">
        <div class="row g-0">
          <div class="col-md-4">
            <img src="admin/resources/images/<?php echo htmlentities($row['image']); ?>" alt="<?php echo htmlentities($row['newstitle']); ?>" class="img-fluid rounded-start">
          </div>
          <div class="col-md-8">
            <div class="card-body">
              <h4 class="card-title" style="color: black"><?php echo htmlentities($row['newstitle']); ?></h4>
              ...
              <p> Posted on <?php echo htmlentities($row['date']); ?></p>
               <p> Time <?php echo htmlentities($row['time']); ?></p>
              <p class="card-text" style="text-align: justify"><?php echo htmlentities($row['newsdescription']); ?></p>
...
<?php } ?>
    ...
    <div class="col-3 justify-content-center text-center px-3 py-3" style="background-color: #9DDFF9; height:fit-content; width: 250px; border-radius:30px; align-items:flex-end; ">
      <div class="row text-center justify-content-center px-5 py-2">
        ...
        </svg>&nbsp;&nbsp;<h3>Announcements</h3>
      </div>
<?php $query = mysqli_query($conn, "SELECT
   announcements.id AS pid,
   announcements.title,
  announcements.content,
   announcements.postdate
...
<div class="col-8"></div>
<div class="col-3 justify-content-center text-center px-3 py-3" style="background-color: #9DDFF9; height: fit-content; width: 250px; border-radius: 30px; margin-top: 10px; align-items: flex-end;">
    <div class="row text-center justify-content-center px-5 py-2">
        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="bi bi-exclamation-square-fill" viewBox="0 0 20 20">
            <path d="M2 0a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V2a2 2 0 0 0-2-2H2zm6 4c.535 0 .954.462.9.995l-.35 3.507a.552.552 0 0 1-1.1 0L7.1 4.995A.905.905 0 0 1 8 4zm.002 6a1 1 0 1 1 0 2 1 1 0 0 1 0-2z"></path>
    ...
    <a href="newsandannouncement.php">All</a>
   
        <?php $query=mysqli_query($conn,"select id,FilterName from newsfilter");
while($row=mysqli_fetch_array($query))
{
?>
