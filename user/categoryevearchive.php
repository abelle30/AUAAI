    ...
    <div class="container">
      ...
      <?php include('resources/includes/sidebar.php'); ?>
        ...
        <div class="col-md-8">
          ...
          <!-- Blog Post -->
          <?php
$query = mysqli_query($conn, "SELECT events.id as pid, events.CategoryId, category.CategoryName
        FROM events 
...
if ($query && mysqli_num_rows($query) > 0) {
    ...
    $categoryName = htmlentities($row['CategoryName']); // Get the CategoryName
} else {
    // Handle the case where no rows were found or there was an error
    $categoryName = "Category not found"; // You can set a default value or handle the error as needed
}
?>

<h1><?php echo $categoryName; ?> Events</h1>
<style>
.card-header{
	color: black;
...
?>
       ...
       <img class="card-img-top" src="../admin/resources/images/<?php echo htmlentities($row['PostImage']);?>" alt="<?php echo htmlentities($row['PostTitle']);?> "height="300px">
            <div class="card-body">
            <h2 class="card-title" style="color: black;"><?php echo htmlentities($row['PostTitle']);?></h2>
