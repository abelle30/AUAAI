...
if (!isset($_SESSION['usertype']) || $_SESSION['usertype'] !== "admin") {
    ...
    $server_name = ""; // Provide your server name here
...
if (isset($_POST['submit']) && isset($_FILES['image'])) {
    ...
    if (isset($_FILES['PostImage']['name']) && $_FILES['PostImage']['name']) {
        ...
        $img_size = $_FILES['PostImage']['size'];
        ...
        if (in_array($img_ex_lc, $allowed_exs)) {
            // Define the absolute server path to the destination directory
            $upload_directory = '/home/u348190438/domains/auaaiconnect.online/public_html/admin/resources/images/';
            ...
            $img_upload_path = $upload_directory . $new_img_name;

            if (move_uploaded_file($tmp_name, $img_upload_path)) {
                // The new file was successfully uploaded.
            } else {
                $_SESSION['msg'] = "File upload failed!";
                // Handle the error or return an error message
                // You might want to exit or redirect to the edit page here
            }
        } else {
            $_SESSION['msg'] = "Invalid file type. Only JPG, JPEG, and PNG files are allowed.";
            // Handle the error or return an error message
            // You might want to exit or redirect to the edit page here
        }
    } else {
        // No new image was uploaded, use the existing image filename if it exists
        $new_img_name = isset($_POST['existing_image']) ? $_POST['existing_image'] : '';

        if (empty($new_img_name)) {
            // Handle the case where "existing_image" is not set or empty
            $_SESSION['msg'] = "No existing image specified.";
            // You might want to exit or redirect to the edit page here
        }
    }
    ...
    $query = mysqli_query($conn, "SELECT * FROM events WHERE id='$pid'");
    $result = mysqli_fetch_row($query);

    $sql = mysqli_query($conn, "UPDATE upcomingevents SET 
    ...
    if ($sql) {
        ...
        $_SESSION['msg'] = "Event update failed!";
    }
}
?>
...
<?php } ?>
                   ...
                   upcomingevents.id,
                   upcomingevents.upcomingevent as upcomingevent,
                   upcomingevents.info as info,
                   upcomingevents.place as place,
                   upcomingevents.date,
                   upcomingevents.time,
                     upcomingevents.category,
                   upcomingevents.image,
                   upcomingevents.description as description
                 
               FROM upcomingevents
               
               LEFT JOIN categoryevents ON categoryevents.id = upcomingevents.CategoryId
               WHERE upcomingevents.id = '$pid'"
              );
...
<form class="form-horizontal row-fluid" name="editevent" method="post" enctype="multipart/form-data">
			...
			<div class="form-group col-md-3">
                    ...
                    <input type="upcomingevent" class="form-control2" id="upcomingevent" name="upcomingevent"  value="<?php echo htmlentities($row['upcomingevent']);?>" required>
                </div>
                
                <div class="form-group col-md-3">
                    <label for="firstname"><b>Date</b></label>
                    <input type="date" class="form-control2" id="date" name="date"  value="<?php echo htmlentities($row['date']);?>" required>
                </div>
                
                   <div class="form-group col-md-3">
                    <label for="firstname"><b>Time</b></label>
                    <input type="time" class="form-control2" id="time" name="time"  value="<?php echo htmlentities($row['time']);?>" required>
                </div>
...
<select class="form-control" name="categoryId" id="categoryId" onChange="getSubCat(this.value);"  required>
<?php
// Feching active categories
$ret=mysqli_query($conn,"select id,CategoryName from  categoryevents ");
...
<?php } ?>
                ...
                <div class="form-group col-md-3">
                    ...
                    <input type="info" class="form-control2" id="info" name="info" value="<?php echo htmlentities($row['info']);?>" required>
                </div>
                <div class="form-group col-md-3">
                    <label for="email"><b>Place</b></label>
                    <input type="place" class="form-control2" id="place" name="place" value="<?php echo htmlentities($row['place']);?>" required>
                </div>


<?php
// Execute your database query here and fetch the row
// For example: $row = mysqli_fetch_assoc($query);

if (isset($row['image'])) {
    $image= htmlentities($row['image']);
} else {
    // Handle the case where 'PostImage' key is not present in the array
    $image = 'default_image.jpg'; // Provide a default image or handle it as needed
}

if (isset($row['title'])) {
    $title = htmlentities($row['title']);
} else {
    // Handle the case where 'title' key is not present in the array
    $title = 'Default Title'; // Provide a default title or handle it as needed
}
?>
...
<img src="/admin/resources/images/<?php echo $image; ?>" id="image" name="image" class="img-fluid rounded-start" width="200" height="150">
<input type="file" name="image" id="image" />  
				...
				<div class="form-group col-md-3">
                    ...
                    <textarea class="summernote" id="description" name="description" required><?php echo htmlentities($row['description']);?></textarea>
                </div>

            <input type="hidden" name="existing_image" value="<?php echo htmlentities($row['image']); ?>">
...
<script>
    function cancelForm() {
        window.location.href = 'https://auaaiconnect.online/admin/manage-events.php';
    }
