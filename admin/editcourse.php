...
// Handle form submission to add a new course or edit an existing course
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST["new_course"])) {
        // Code for adding a new course remains unchanged
    } elseif (isset($_POST["edit_course_id"]) && isset($_POST["edited_course_name"])) {
        $editCourseId = $_POST["edit_course_id"];
        $editedCourseName = $_POST["edited_course_name"];

        // Use prepared statement to prevent SQL injection
        $stmt = $conn->prepare("UPDATE courses SET course_name=? WHERE id=?");
        $stmt->bind_param("si", $editedCourseName, $editCourseId);

        if ($stmt->execute()) {
            echo '<script>alert("Course edited successfully");</script>';

            // Wait for 2 seconds (adjust the time as needed)
            header("refresh:0.1;url=manage_courses.php");
    ...
    <div class="container">
        <div class="row d-flex align-items-center flex-wrap">
            <div class="col-md-7">
                ...
                <h1 class="h2"><b>Edit Course</b></h1>
            </div>
            </ul>
            </div>
        </div>
...
<div class="card">
    ...
    <div class="card-header">
        <h2 class="alumniList">Edit</h2>
    </div>


    <br>
    <?php
// Fetch the user's course details based on the provided user ID
if (isset($_GET['id'])) {
    $user_id = $_GET['id'];

    // Fetch the course details based on the user ID
    $course_info_query = mysqli_query($conn, "SELECT * FROM courses WHERE id = $user_id");
    $course_info = mysqli_fetch_assoc($course_info_query);

    // Check if the course exists
    if ($course_info) {

        echo "<h3>Course Name: " . $course_info['course_name'] . "</h3>";

        // Fetch all records for the specified course using its ID
        $course_records_query = mysqli_query($conn, "SELECT * FROM courses WHERE id = '$user_id'");

        if (mysqli_num_rows($course_records_query) > 0) {
            // Process and display the course records as needed
        }
    } else {
...
<form class="form-horizontal row-fluid" name="editcourse" method="post" action="manage_courses.php">
    <div class="form-group col-md-3">
        <input type="hidden" name="edit_course_id" value="<?php echo $user_id; ?>">
        <br>
        <label for="edited_course_name"><b>New Course Name:</label>
        <input type="text" name="edited_course_name" class="form-control" required>
        <br>
        <button class="btn btn-primary" type="submit" name="edit_course">Edit Course</button>
    </div>
</form>
