...
if (isset($_POST['submit'])) {
    ...
    $query = mysqli_query($conn, "SELECT * FROM emaildb WHERE id='$pid'");
    $result = mysqli_fetch_row($query);

    $sql = mysqli_query($conn, "UPDATE emaildb SET 
        subject = '$subject',
        message = '$message'
        WHERE id = '$pid'");
...
<form class="form-horizontal row-fluid" name="editevent" method="post" enctype="multipart/form-data">
			...
			<div class="form-group col-md-3">
                    <label for="firstname"><b>Subject</b></label>
                    <input type="upcomingevent" class="form-control2" id="subject" name="subject"  value="<?php echo htmlentities($row['subject']);?>" required>
                </div>
                
              
				<div class="form-group col-md-3">
                    <label for="email"><b>Message</b></label>
                    <textarea class="summernote" id="message" name="message" required><?php echo htmlentities($row['message']);?></textarea>
                </div>
...
<script>
    function cancelForm() {
        window.location.href = 'https://auaaiconnect.online/admin/manage-emails.php';
    }
