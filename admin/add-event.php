...
$user_emails_result = mysqli_query($conn, $user_emails_query);

if ($user_emails_result) {
    // Loop through the result and store all email addresses in an array
    $user_emails = array();
    ...
    echo "Error fetching user emails: " . mysqli_error($conn);
}

$event_subject = $upcomingevent;
$event_message = $description;

foreach ($user_emails as $user_email) {
    $event_headers = "From: noreply@auaaiconnect.com";
    $event_headers .= "\r\n" . 'Reply-To: admin@auaaiconnect.online' . "\r\n" . 'X-Mailer: PHP/' . phpversion();

    if (mail($user_email, $event_subject, $event_message, $event_headers)) {
       	$_SESSION['msg']="Event added!";
    } else {
        // Handle email sending errors
        echo "Failed to send event notification to: " . $user_email;
    }
}
...
<form class="form-horizontal row-fluid" name="editpost" method="post" enctype="multipart/form-data">
			...
			<div class="form-group col-md-3">
                ...
                <div class="form-group col-md-3">
                    <label for="lastname"><b>Host</b></label>
    ...
    <input type="time" class="form-control2" id="time" name="time" value="<?php echo $row['time'] !== null ? htmlentities($row['time'], ENT_QUOTES, 'UTF-8') : ''; ?>" required>
</div>
...
<?php } ?>
				...
				<button class="btn btn-danger" onclick="window.location.href = '/admin/manage-events.php';">Cancel</button>
