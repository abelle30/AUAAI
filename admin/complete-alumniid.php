...
exit();
    ...
    if(isset($_GET['del']) && isset($_GET['id'])) {
        $id = $_GET['id'];
        mysqli_query($conn, "DELETE FROM alumniid WHERE id = '$id'");
        $_SESSION['delmsg'] = "Event with ID $id deleted!";
         ...
         $alumni_id = $_GET['id'];
        $email = $_GET['PersonalEmailAddress'];
$sql = "UPDATE alumniid SET status = 0 WHERE id = $alumni_id";


if (mysqli_query($conn, $sql)) {
    // Successful update

    $subject = "Alumni ID Ready to Pick Up";
  $message = "Good day our dear Alumni! Your alumni ID is ready to pick up. Just visit the Alumni Office at Adamson University Alumni Office, Ground Floor SV Building, 900 San Marcelino Street, Ermita 1000 Manila, Philippines every Monday - Friday from 8 AM to 5 PM.\n\nThank you and have a nice day!\n\nBest regards,\nAdamson University Alumni Association Inc.";

    $headers = "From: noreply@auaaiconnect.online";

    // Additional headers should be added using the .= operator, not overwriting the previous $headers
    $headers .= "\r\n" .
    ...
    // You need to fetch the user's email address here. Assuming you have it in a variable $email.
// Replace with the actual user's email address

    if (mail($email, $subject, $message, $headers)) {
        // Start a session and set a success message
        session_start();
        $_SESSION['msg'] = "ID completed!";
        header("Location: https://auaaiconnect.online/admin/verifylist.php");
        exit();
    } else {
        // Start a session and set an error message
        session_start();
        $_SESSION['error'] = "Failed to complete ID.";
    }
} else {
    // Error handling
        ...
        <div class="container">
            <div class="row d-flex align-items-center flex-wrap">
                <div class="col-md-7">
                    ...
                    <h1 class="h2"><b>Manage Alumni ID</b></h1>
                </div>
        ...
        <div class="card-body">
                ...
                <div class="container">
                        ...
                        <div class="card-header">
                            <h2 class="alumniList"> Manage Alumni ID </h2>
                        </div>
                        ...
                        <table cellpadding="0" cellspacing="0" border="0" class="datatable-1 table table-bordered table-striped	 display" width="100%">
                                        ...
                                        <th class="text-center">Position in the company</th>
                                        <th class="text-center">Type of Card</th>
...
$query = mysqli_query($conn, "SELECT alumniid.id AS id, alumniid.idpic AS idpic,
                                alumniid.alumniname AS alumniname,alumniid.YearGraduated AS YearGraduated, alumniid.StudentNumber AS StudentNumber,  
                                alumniid.Course AS Course, alumniid.CompanyName AS companyname,
                                alumniid.CompanyAddress AS CompanyAddress,alumniid.Position AS Position,
                                alumniid.Birthday AS Birthday,alumniid.typeofcard AS typeofcard,alumniid.proofofpayment AS proofofpayment,
                                alumniid.esignature AS esignature
                                FROM alumniid WHERE alumniid.status=1 ");
    ...
    <td><?php echo $cnt; ?></td>  
    <td><img src="resources/images/<?php echo  $row['idpic']; ?>" class="img-fluid rounded-start"> </td>
    <td><?php echo $row['alumniname']; ?></td>  
    ...
    <td><?php echo $row['typeofcard']; ?></td>  
    <td><img src="resources/images/<?php echo  $row['proofofpayment']; ?>" class="img-fluid rounded-start"> </td>
    <td><img src="resources/images/<?php echo  $row['esignature']; ?>" class="img-fluid rounded-start"> </td>


    <td>
    <a tittle="Complete" href="complete-alumniid.php?id=<?php echo $row['id']; ?>"><i class="icon-edit">Complete</i></a>
    <a tittle="Remove Products" href="managealumniid.php?id=<?php echo $row['id']; ?>&del=delete" onClick="return confirm('Are you sure you want to delete?')">Delete</a>
    </td>
    </tr>  
    <?php $cnt++;
    } ?>	
    ...
    <?php } ?>
        ...
        <!-- Import & Export link -->
                ...
                <a href="javascript:void(0);" class="btn btn-success" onclick="formToggle('importFrm');"><i class="plus"></i> Import a CSV file</a>
                ㅤ

                <a href="exportcsvfile-products.php" class="btn btn-primary"><i class="exp"></i> Export CSV file</a>
        
        <!-- CSV file upload form -->
        <div class="col-md-12" id="importFrm" style="display: none;">
            <form action="importcsvfile-products.php" method="post" enctype="multipart/form-data">
