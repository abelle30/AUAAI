...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Get the user's submitted email
    $email = $_POST["email"];

    // Validate the email (you can use more robust email validation)
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        $_SESSION['error'] = "Invalid email address";
        header("Location: forgotpassworduser.php"); // Redirect back to the forgot password page
        exit();
    }
    ...
    try {
        ...
        // Generate a unique token for the password reset link (securely)
        $reset_token = bin2hex(random_bytes(32));

        // Calculate the expiration timestamp (e.g., 1 hour from now)
        $expiration_timestamp = date('Y-m-d H:i:s', strtotime('+1 hour'));

        // Insert the user's email, reset token, and expiration timestamp into the database
        $insert_stmt = $pdo->prepare("INSERT INTO reset_tokens (user_email, reset_token, expiration_timestamp) VALUES (:user_email, :reset_token, :expiration_timestamp)");
        $insert_stmt->bindParam(':user_email', $email, PDO::PARAM_STR);
        $insert_stmt->bindParam(':reset_token', $reset_token, PDO::PARAM_STR);
        $insert_stmt->bindParam(':expiration_timestamp', $expiration_timestamp, PDO::PARAM_STR);
        $insert_stmt->execute();

        // Construct the reset link
        $reset_link = "https://auaaiconnect.online/reset-password.php?email=" . urlencode($email) . "&token=" . $reset_token;
        ...
        // Additional headers
        $headers .= "\r\nReply-To: noreply@auaaiconnect.com";
        $headers .= "\r\nContent-Type: text/plain; charset=UTF-8";

        if (mail($email, $subject, $message, $headers)) {
            $_SESSION['success'] = "Password reset instructions sent to your email.";
        } else {
            $_SESSION['error'] = "Failed to send password reset instructions. Please try again later.";
        }
...
?>
            ...
            <div class="bg-image"  style="background-image: (resources/images/ADAMSON-1.jpg); height: 100vh;">   
                ...
                <br>
                    ...
                    <div class="container justify-content-center">
                            ...
                            <p class="pmessage" style="color: white;">Please provide the email address that you used when you signed up for your account.</p>
                        </div>
                        <br><br>
                    </div>
                </div>
                            ...
                            <div class="card-body">
                                ...
                                <?php if (isset($_SESSION['success'])) { ?>
                                    <div class="alert alert-success">
                                        <button type="button" class="close" data-dismiss="alert">×</button>
                                        <strong>Sent!</strong> <?php echo htmlentities($_SESSION['success']); ?>
                                    </div>
                                    <?php unset($_SESSION['success']); // Clear the error message ?>
                                <?php } ?>

                                <div class="d-flex justify-content-center">
                                    <div class="form-group">
                                        <button class="btn btn-primary profile-button" name="submit_email" type="submit">Submit</button>
                                    </div>
                                </div>
                            </div>
                        </div>
